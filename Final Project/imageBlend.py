...
def expand(image):
  ...
  image2 = np.zeros((width*2, height*2))
  image2[::2,::2] = image
  kernel = generatingKernel(0.4)
  image3 = 4 * scipy.signal.convolve2d(image2, kernel, 'same')
  return image3
...
def laplPyramid(gaussPyr):
  ...
  for k in range(len(gaussPyr) - 1):
    shape = gaussPyr[k].shape
    nextImage = expand(gaussPyr[k+1])
    if nextImage.shape == shape:
      output.append(gaussPyr[k] - nextImage)
    else:
      next = nextImage[0:shape[0], 0:shape[1]] 
      output.append(gaussPyr[k] - next)
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  for i in range(len(laplPyrWhite)):
    mask = (1 - gaussPyrMask[i])
    result =  gaussPyrMask[i] * laplPyrWhite[i] + mask * laplPyrBlack[i]
...
def collapse(pyramid):
  ...
  """
  # WRITE YOUR CODE HERE.
  max = pyramid[len(pyramid) - 1]
  for i in reversed(range(len(pyramid) - 1)):
    nextImage = expand(max)
    if nextImage.shape == pyramid[i].shape:
      max = pyramid[i] + nextImage
    else:
      next = nextImage[0:pyramid[i].shape[0], 0:pyramid[i].shape[1]] 
      max = pyramid[i] +  next
  return max
  # END OF FUNCTION.