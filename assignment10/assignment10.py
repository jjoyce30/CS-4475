...
def getYXLocations(image, intensity_value):
    ...
    """
    # WRITE YOUR CODE HERE.
    locs = np.argwhere(image == intensity_value)
    x_locs = np.zeros(len(locs),dtype = np.int64)
    y_locs = np.zeros(len(locs),dtype = np.int64)
...
def computeResponseCurve(pixels, log_exposures, smoothing_lambda,
    ...
    for i in xrange(pix_range):
        for j in xrange(num_images):
            ...
            mat_A[idx_ctr][pix_range+1]= (-1) * wij
    ...
    a_inverse = np.linalg.pinv(mat_A)
    x = np.dot(a_inverse, mat_b)
...
#exposure_times = np.float64([1/200.0, 1/160.0, 1/125.0, 1/80.0, 1/60.0, 1/40.0])
