...
def sumSquaredDifferences(video_volume):
    ...
    for i in range (len(video_volume)):
        cur_frame = video_volume[i].astype(np.float32)
        for j in range (len(video_volume)):
            comparison_frame = video_volume[j].astype(np.float32)
            ssd = np.sum((cur_frame - comparison_frame)** 2)
            output[i,j] = ssd
    # END OF FUNCTION.
    return output
...
def transitionDifference(ssd_difference):
    ...
    # WRITE YOUR CODE HERE.
    binomialFilter = binomialFilter5()
    kern = np.diag(binomialFilter)
    output = scipy.signal.convolve2d(ssd_difference, kern, mode='valid')
...
def findBiggestLoop(transition_diff, alpha):
    ...
    # WRITE YOUR CODE HERE.
    largest_score = alpha * (end - start) - transition_diff[end, start]
    for i in range (transition_diff.shape[0]):
        for j in range(transition_diff.shape[1]):
            curr = alpha * (j - i) - transition_diff[j, i]
            if curr > largest_score:
                start = i
                end = j
                largest_score = curr
