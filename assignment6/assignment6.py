...
def reduce(image):
  ...
  image2 = scipy.signal.convolve2d(image, kernel, 'same')
  img = image2[::2, ::2]
...
def expand(image):
  ...
  kernel = generatingKernel(0.4)
  imageZero = np.zeros((width * 2, height * 2))
  imageZero[::2, ::2] = image
  image2 = scipy.signal.convolve2d(imageZero, kernel, 'same')
  image3 = 4 * image2
  return (image3)
...
def laplPyramid(gaussPyr):
  ...
  output = []
  # WRITE YOUR CODE HERE.
  for k in range (len(gaussPyr) -1):
      tryIt = expand(gaussPyr[k + 1])
      original = gaussPyr[k].shape
      if tryIt.shape == original:
        expanded1 = gaussPyr[k] - tryIt
        output.append(expanded1)
      else:
          #tryIt[0:original[0], 0:original[1]]
        newTry = tryIt[0:original[0], 0:original[1]]
        expanded = gaussPyr[k] - newTry
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  for i in range(len(laplPyrBlack)):
    blender = gaussPyrMask[i] * laplPyrWhite[i] + (1 - gaussPyrMask[i]) * laplPyrBlack[i]
    blended_pyr.append(blender)
  return blended_pyr
...
def collapse(pyramid):
  ...
  """
  # WRITE YOUR CODE HERE.
  original = pyramid[len(pyramid) - 1]
  for i in reversed(range(len(pyramid) - 1)):
    wanted = pyramid[i].shape
    down = expand(original)
    if down.shape == wanted:
        original = pyramid[i] + down
    else:
        print (down.shape)
        original = pyramid[i] + down[0:wanted[0], 0:wanted[1]]
  return original
