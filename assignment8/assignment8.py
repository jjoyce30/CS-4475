...
def getImageCorners(image):
    ...
    corners = np.float32([ [[0,0]], [[0, width]], [[height, 0]], [[height, width]] ])
    return corners
...
def findHomography(image_1_kp, image_2_kp, matches):
    ...
    for x,y in enumerate(matches):
        image_1_points[x] = image_1_kp[y.queryIdx].pt
        image_2_points[x] = image_2_kp[y.trainIdx].pt
        homeography = np.float64
...
def blendImagePair(warped_image, image_2, point):
    ...
    second = (image_2 * 0.85) + output_image[point[1]:point[1] +image_2.shape[0], point[0]:point[0] + image_2.shape[1]]* 0.15
    output_image[point[1]:point[1] + image_2.shape[0],
                 point[0]:point[0] + image_2.shape[1]] = (second)
...
def warpImagePair(image_1, image_2, homography):
    ...
    warped_image = cv2.perspectiveTransform(corners_1, homography)
    doubleCorners = np.append(warped_image, corners_2, axis = 0)
    mins = np.amin(doubleCorners, axis = 0)
    x_min = mins[:,0]
    y_min = mins[:,1]
    maxes = np.amax(doubleCorners, axis = 0)
    x_max = maxes[:,0]
    y_max = maxes[:,1]
    translation = np.array( [[1, 0, -x_min],
                             [0, 1, -y_min],
                             [0, 0, 1]] )
    dot_product = np.dot(translation, homography)
    warped_image = cv2.warpPerspective(image_1, dot_product,(x_max - x_min, y_max - y_min))
